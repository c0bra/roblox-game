...
export const appShell = (): string => `
  <main class="game-shell" data-screen="select">
    ...
    <section class="select-screen" id="select-screen" aria-labelledby="game-title">
      ...
      <h1 id="game-title" tabindex="-1">Heaven’s <em>Edge</em></h1>
      <p class="premise">Hold the line between sacred light and the void. Choose the sound that becomes your weapon.</p>
      <div class="instrument-grid" role="radiogroup" aria-label="Choose your instrument">
        ${instruments
          .map(
            (instrument, index) => `
          <button class="instrument-card${index === 0 ? " is-selected" : ""}" type="button" role="radio" aria-checked="${index === 0}" data-instrument="${instrument}">
    ...
    <section class="overlay-card result-card" id="result-overlay" aria-labelledby="result-title" hidden>
      ...
      <button class="text-button" id="change-button" type="button">Change instrument</button>
    </section>
    ...
    <section class="overlay-card" id="error-overlay" aria-labelledby="error-title" hidden>
      ...
      <button class="text-button" id="error-back-button" type="button">Choose another instrument</button>
    </section>
  </main>`;
